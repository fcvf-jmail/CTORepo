...
3. **Этап final** - создание финального образа на базе runtime образа (`mcr.microsoft.com/dotnet/aspnet:8.0`)
...
Миграции Entity Framework Core применяются автоматически при запуске приложения из `Program.cs`:

1. При старте приложения создается scope для сервисов
2. Получается экземпляр `ApplicationDbContext`
3. Вызывается метод `db.Database.Migrate()` для применения миграций
4. После успешного применения миграций запускается API

Этот подход обеспечивает:
- Автоматическое применение миграций без необходимости в отдельных скриптах
- Интеграцию миграций непосредственно в код приложения
...
Текущая конфигурация включает production-ready подходы:

1. ✅ Используется легковесный образ `aspnet:8.0` runtime вместо SDK
2. ✅ Миграции применяются автоматически из кода приложения
3. ✅ Healthcheck для PostgreSQL
4. ✅ Restart policy для автоматического восстановления

Для полноценного production окружения рекомендуется:

1. Настроить логирование в внешнее хранилище (например, ELK stack)
...
6. Добавить reverse proxy с SSL/TLS (nginx, traefik)

## Связанная документация

- [PROGRAM_CS_MIGRATIONS.md](PROGRAM_CS_MIGRATIONS.md) - Детальная информация о применении миграций из Program.cs
- [DATABASE.md](DATABASE.md) - Работа с базой данных и миграциями
